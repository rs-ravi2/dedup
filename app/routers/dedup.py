...
async def store_record(
    ...
    try:
        ...
        await dedup_service.store_customer(transaction_id, image_data, store_metadata)
...
async def search_similar(
    ...
    threshold: Optional[float] = Form(
        default=0.6, description="Similarity threshold (0.0-1.0)"
    ),
    ...
    try:
        ...
        results = await dedup_service.search_similar_customers(
            image_data, threshold or 0.6, limit
        )
...
async def purge_record(request: PurgeRequest, token: str = Depends(verify_token)):
    """
...
async def health_check():
    ...
    try:
        ...
        overall_healthy = health_status["overall"] and model_status.get(
            "initialized", False
        )
